...
**Local task** — a granular checklist item internal to Claude Code, stored in ~/.claude/tasks/. These are the agent's working memory during implementation. They also represent a natural commit point, with each completed task representing an individual commit to version control. The human does not need to inspect them during work execution. They exist so the agent can track its own progress within and across sessions, manage dependencies between steps, and resume after context compaction without losing its place.
